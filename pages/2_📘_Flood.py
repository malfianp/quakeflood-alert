...
# function tab 1
def reset_cb_tab3():
        st.session_state.cb_tab3 = False
...
with tab3:
    ...
    if st.checkbox("Confirm datetime?", key="cb_tab3"):
        # proses get info
        # klasifikasi
...
_#QuakeFloodAlert_"""
        ...
        # tabs
        st.write("**Send to a group (Telegram bot):**")

        date_name=datetime3.replace(":", "-")
        path_image=f"fig/fig {date_name}.png"

        # telegram bot
        async def send_photo(bot, chat_id, image, caption, message):
            caption = message
            await bot.sendPhoto(chat_id, image, caption, parse_mode='MARKDOWN')
        ...
        if st.checkbox("Preview? "):
            try:
                with stylable_container(key="container_with_border",
                                        ...
                                        border-radius: 0.5rem;
                                        padding: calc(1em - 1px)}"""):
                    st.write(f"**Receiver** : Telegram Group ['QuakeFlood Alert']")
                    st.write("**Image** : ")
                    st.pyplot(fig3)
                    st.write("**Caption** : ")
                    st.write(message)
        ...
        if st.button("Send Message ", type="primary"):
            try:
                ...
                caption = " "
                # Send 
                asyncio.run(send_photo(bot, chat_id, image, caption, message))
        ...
        # reset button
        "---"      
        st.button(" Click to reset   ", on_click=reset_cb_tab3)